...
public final class UserValidator {
	...
	public static Map<String, Boolean> getValidationResults(UserSignUpDto user) {
		...
		validationResult.put(
				...
				UserFieldValidator.getValidatorByFieldName(EntityColumn.UserDetail.NAME.getName()).isValid(user.getFirstName()));

		validationResult.put(
				EntityColumn.UserDetail.SURNAME.getName(),
				UserFieldValidator.getValidatorByFieldName(EntityColumn.UserDetail.SURNAME.getName()).isValid(user.getLastName()));
	...
	private static void putValidationResultsOfEmailAndPasswordFields(
		...
		validationResult.put(
				...
				UserFieldValidator.getValidatorByFieldName(EntityColumn.User.EMAIL.getName()).isValid(email));

		validationResult.put(
				EntityColumn.User.PASSWORD.getName(),
				UserFieldValidator.getValidatorByFieldName(EntityColumn.User.PASSWORD.getName()).isValid(password));
	}
